...
Item {
  ...
  readonly property string configPath: root.controller.configPath
  property bool _savingConfig: false

  // One instance owns interaction and autohide for one monitor. The controller
  // keeps session state and IPC alive when monitor selection changes.
  required property var controller
  property var dockScreen: null
  property bool primary: false
  property string screenName: ""
  readonly property var availableScreens: Quickshell.screens
  readonly property var appLibrary: !root.primary ? root.controller.appLibrary
    : ((shell && shell.appLibrary) ? shell.appLibrary : localAppLibrary)
  ...
  readonly property var minimizedOrigins: root.controller.minimizedOrigins
  readonly property var parkedAt: root.controller.parkedAt
  readonly property var urgentMap: root.controller.urgentMap
  readonly property var recentOpenedWindowAddrs: root.controller.recentOpenedWindowAddrs
  ...
  // moment focus moves.
  readonly property var appRecentWindow: root.controller.appRecentWindow

  // Apps whose launch has been asked for but whose window has not shown up yet.
  readonly property var launchPending: root.controller.launchPending
  ...
  Connections {
    ...
    function onLaunchPendingChanged() {
      modelTimer.restart()
      if (root.primary && Object.keys(root.launchPending).length > 0) launchPruneTimer.start()
    }
    function onUrgentMapChanged() { modelTimer.restart() }
    function onMinimizedOriginsChanged() { modelTimer.restart() }
  }
      ...
      try {
        ...
        if (top && top.appId) {
          ...
          if (aid && address) {
            ...
            recent[aid] = address
            root.controller.appRecentWindow = recent
          }
  ...
  Connections {
    ...
    function onRawEvent(event) {
      ...
      if (n === "openwindow") {
        ...
        root.controller.recentOpenedWindowAddrs = rec
      }
      if (n === "urgent") {
      ...
      if (n === "closewindow") {
        ...
        if (root.minimizedOrigins && root.minimizedOrigins[fullAddr]) {
          ...
          delete mo[fullAddr]
          root.controller.minimizedOrigins = mo
        }
        root.refreshDock()
      }
  ...
  function updateNotifService() {
    if (root.primary && !root.notifService && root.shell && typeof root.shell.serviceFor === "function") {
  ...
  Timer {
    ...
    interval: 200
    repeat: true
    running: root.primary && !root.notifService && root._notifServiceAttempts < 25
    onTriggered: {
  ...
  function handleThemeChanged() {
    ...
    root.themeVersion++
    if (root.primary && root.appLibrary) {
  ...
  function minimizeToplevel(topOrAddr) {
    ...
    root.controller.minimizedOrigins = origins

    var parkedTimes = DockModel.copyMap(root.parkedAt)
    parkedTimes[address] = Date.now()
    root.controller.parkedAt = parkedTimes
  ...
  function pruneWindowState() {
    ...
    root.controller.minimizedOrigins = root.keepLive(root.minimizedOrigins, live, false)
    root.controller.parkedAt = root.keepLive(root.parkedAt, live, false)
    root.controller.appRecentWindow = root.keepLive(root.appRecentWindow, live, true)
    root.controller.urgentMap = root.keepUrgentLive(root.urgentMap, live)
    ...
    if (roaChanged) root.controller.recentOpenedWindowAddrs = nextRoa
  }
  ...
  function restoreLast() {
    ...
    var all = root.pinnedSection.concat(root.runningSection)
    for (var i = 0; i < all.length; i++) {
      if (all[i]) parked = parked.concat(root.parkedWindows(all[i].windowList || []))
    }
  ...
  function markLaunching(appId, windowsBefore) {
    ...
    root.controller.launchPending = pending
    // The primary instance prunes shared launch state for every monitor.
  }
  ...
  function pruneLaunching() {
    ...
    if (changed) root.controller.launchPending = next
